...
@pytest.mark.parametrize('mock_sudoku_load', [dict(file_name='naked_quadruples.txt')], indirect=True)
def test_integrationtest_naked_quadruples_collection(mock_sudoku_load):
    capturedOutput = io.StringIO()
    sys.stdout = capturedOutput
    sudoku_strategy = strategies.collectionstrategy.CollectionStrategy(sudoku=mock_sudoku_load.sudoku,
                                                                       candidates=mock_sudoku_load.candidates,
                                                                       constraints=mock_sudoku_load.constraints,
                                                                       strategy='naked_quadruples',
                                                                       units=['row', 'column', 'box'])
    output_found = sudoku_strategy.detect()
    sys.stdout = sys.__stdout__
    assert capturedOutput.getvalue() == 'Naked Quadruple (3, 4, 8, 9) found in row 4\n' \
                                        'Naked Quadruple (1, 3, 4, 5) found in row 5\n' \
                                        'Naked Quadruple (4, 5, 6, 8) found in row 7\n' \
                                        'Naked Quadruple (2, 3, 5, 8) found in column 4\n' \
                                        'Naked Quadruple (3, 4, 5, 8) found in column 5\n' \
                                        'Naked Quadruple (3, 4, 7, 9) found in column 7\n' \
                                        'Naked Quadruple (3, 4, 6, 7) found in column 8\n' \
                                        'Naked Quadruple (1, 3, 5, 8) found in box 4\n' \
                                        'Naked Quadruple (4, 5, 6, 8) found in box 6\n' \
                                        'Naked Quadruple (3, 4, 5, 8) found in box 7\n'
    assert output_found == [
